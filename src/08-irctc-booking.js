...
export async function checkSeatAvailability(trainNumber, date, classType) {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      if (!trainNumber || typeof trainNumber !== 'string' || !/^\d{5}$/.test(trainNumber)) {
      ...
      resolve({ trainNumber, date, classType, available: seats > 0, seats, waitlist });
    }, 100);
  });
}

export async function bookTicket(passenger, trainNumber, date, classType) {
  if (!passenger || !passenger.name || passenger.age === undefined || !passenger.gender) {
  ...
  if (availability.available) {
    ...
    return {
      pnr: "PNR" + Math.floor(Math.random() * 1000000),
      passenger, trainNumber, date, class: classType, status: "confirmed", fare: fares[classType]
    };
  } else {
    return { status: "waitlisted", waitlistNumber: Math.floor(Math.random() * 20) + 1 };
  }
}
...
export async function cancelTicket(pnr) {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      resolve({ pnr, status: "cancelled", refund: Math.floor(Math.random() * 901) + 100 });
    }, 50);
  });
}
...
export async function getBookingStatus(pnr) {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      resolve({ pnr, status: statuses[Math.floor(Math.random() * 3)], lastUpdated: new Date().toISOString() });
    }, 50);
  });
}
