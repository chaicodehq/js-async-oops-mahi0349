...
export async function processPayment(amount, balance, networkStatus) {
  return new Promise((resolve, reject) => {
    if (typeof amount !== 'number' || amount <= 0) {
      return reject(new PaymentError("Invalid amount", "INVALID_AMOUNT", amount));
    }
    if (amount > balance) {
      return reject(new InsufficientFundsError(amount, balance));
    }
    if (networkStatus === "offline") {
      return reject(new NetworkError(amount));
    }
    if (amount > 100000) {
      return reject(new FraudDetectedError(amount));
    }
    setTimeout(() => {
      resolve({
        transactionId: "TXN" + Math.floor(Math.random() * 1000000),
