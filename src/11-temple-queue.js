...
export class TempleQueue {
  ...
  enqueue(name, type) {
    if (!name || (type !== "regular" && type !== "vip")) return null;
  ...
  static merge(queue1, queue2) {
    const newQueue = new TempleQueue(`${queue1.templeName}-${queue2.templeName}`, queue1.length + queue2.length + 10);
    newQueue.#devotees = [...queue1.#devotees, ...queue2.#devotees];
    return newQueue;
  }
