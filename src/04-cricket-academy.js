...
export class Batsman extends Player {
  ...
  getProfile() {
    ...
    return { ...super.getProfile(), battingStyle: this.battingStyle, role: "batsman", totalRuns, inningsPlayed: this.innings.length };
  }
}
...
export class AllRounder extends Player {
  ...
  getProfile() {
    const totalRuns = this.innings.reduce((sum, i) => sum + i.runs, 0);
    const totalWickets = this.spells.reduce((sum, s) => sum + s.wickets, 0);
    return { ...super.getProfile(), battingStyle: this.battingStyle, bowlingStyle: this.bowlingStyle, role: "allrounder", totalRuns, totalWickets, inningsPlayed: this.innings.length, spellsBowled: this.spells.length };
  }
}
