...
LassiStand.prototype.addFlavor = function(flavor, price) {
  if (price <= 0 || this.menu.some(m => m.flavor === flavor)) return -1;
  this.menu.push({ flavor, price });
  return this.menu.length;
};

LassiStand.prototype.takeOrder = function(customerName, flavor, quantity) {
  const flavorObj = this.menu.find(m => m.flavor === flavor);
  if (!flavorObj || quantity <= 0) return -1;
  const orderId = this._nextOrderId++;
  this.orders.push({ id: orderId, customer: customerName, flavor, quantity, total: flavorObj.price * quantity, status: "pending" });
  return orderId;
};
